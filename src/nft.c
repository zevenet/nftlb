...
static int run_base_chain(struct sbuffer *buf, struct nftst *n, int type, int family, unsigned int rules_needed, int action)
{
	...
	if (action == ACTION_STOP || action == ACTION_DELETE) {
		...
		if (((~type & NFTLB_F_CHAIN_POS_SNAT) && (~type & NFTLB_F_CHAIN_PRE_DNAT) && (*base_rules & NFTLB_IP_ACTIVE) &&
			...
			((~type & NFTLB_F_CHAIN_POS_SNAT) && (type & NFTLB_F_CHAIN_PRE_DNAT) && (*base_rules & NFTLB_IP_ACTIVE) &&
			...
			*base_rules &= ~NFTLB_IP_ACTIVE;
			...
			if (type & NFTLB_F_CHAIN_PRE_DNAT) {
				...
				*base_rules &= ~NFTLB_IP_ACTIVE & ~NFTLB_PROTO_IP_PORT_ACTIVE & ~NFTLB_PROTO_PORT_ACTIVE & ~NFTLB_PROTO_IP_ACTIVE & ~NFTLB_MARK_ACTIVE;
			}
		}
	}
...
static void run_nftst_rules_gen_srv_map_by_type(struct sbuffer *buf, struct nftst *n, int type, int family, int protocol, int action)
{
	...
	if (elements && (~type & NFTLB_F_CHAIN_POS_SNAT))
...
static int run_nftst_rules_ingress_policies(struct sbuffer *buf, struct nftst *n, char *chain, int action)
{
	...
	list_for_each_entry(ap, &a->policies, list) {
		if (ap->policy->type != VALUE_TYPE_ALLOW)
			continue;
		...
		run_farm_rules_log_and_verdict(buf, n, a->logrtlimit, a->verdict, VALUE_TYPE_ALLOW, KEY_LOGPREFIX, NFTLB_F_CHAIN_ING_FILTER);
		ap->action = ACTION_NONE;
	}

	list_for_each_entry(ap, &a->policies, list) {
		if (ap->policy->type != VALUE_TYPE_DENY)
			continue;

		concat_buf(buf, " ; add rule %s %s %s %s saddr @%s",
						NFTLB_NETDEV_FAMILY_STR, NFTLB_TABLE_NAME, chain, print_nft_family(ap->policy->family), ap->policy->name);
		run_farm_rules_log_and_verdict(buf, n, a->logrtlimit, a->verdict, VALUE_TYPE_DENY, KEY_LOGPREFIX, NFTLB_F_CHAIN_ING_FILTER);
		ap->action = ACTION_NONE;
	}
...
static int run_farm_rules_gen_nat(struct sbuffer *buf, struct nftst *n, int family, int type, int action)
{
	...
	default:
		...
		concat_buf(buf, " ; add rule %s %s %s %s %s %s dnat", print_nft_table_family(family, NFTLB_F_CHAIN_PRE_DNAT), NFTLB_TABLE_NAME, chain, print_nft_family(family), print_nft_family_protocol(family), print_nft_protocol(nftst_get_proto(n)));
